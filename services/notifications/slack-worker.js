...
// Use api.blackroad.io (custom domain) to avoid CF same-zone subrequest block
const AI_API = 'https://api.blackroad.io/v1/chat/completions';
const OLLAMA_DIRECT = 'https://api.blackroad.io';
...
async function askAgent(agentId, message, context = [], env = null) {
  ...
  try {
    const reqInit = {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-BlackRoad-Chat': 'true' },
      body: JSON.stringify({ model: agent.model, messages, max_tokens: 80 }),
    };

    // Service Binding bypasses CF same-zone restriction
    const res = env?.AI_GATEWAY
      ? await env.AI_GATEWAY.fetch('https://fake/v1/chat/completions', reqInit)
      : await fetch(AI_API, reqInit);
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-BlackRoad-Chat': 'true' },
      body: JSON.stringify({ model: agent.model, messages, max_tokens: 80 }),
    });
    ...
    if (data.error) {
      return `(${agent.name}: ${data.error.message || 'gateway error'})`;
    }
    return '...';
  } catch (e) {
